...
public class CustomerRegister {
	...
	public void register() {
		if (verifyCodeIsAvailable()) {
			...
			messages.info(new DefaultBundleKey("customer_registered"))
					...
					.params(newCustomer.getCodeCustomer());
			log.customerConfirmed(newCustomer.getCodeCustomer() + "");
		} else {
	...
	private boolean verifyCodeIsAvailable() {
		Customer existing = em.find(Customer.class,
				newCustomer.getCodeCustomer());
		if (existing != null) {
			messages.warn(new BundleKey("messages", "account_codeTaken"))
					.defaults(
							"The code '{0}' is already taken. Please choose another code.")
					.targets(codeInput.getClientId())
					.params(newCustomer.getCodeCustomer());
			log.customerAvailable(existing.getCodeCustomer() + "",
					existing != null);
			return false;
		}
